...
const CardFeature = ({ image, name, price, category, loading, id }) => {
  ...
  return (
    <div className='w-full min-w-[200px] max-w-[200px]  bg-white hover:drop-shadow-lg px-4 py-4 cursor-pointer flex flex-col '>
      {
        image
          ?
          <>
            <Link to={`/menu/${id}`} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
              <div className='h-28 flex flex-col justify-center item-center'>
                <img src={image} className='h-full' alt='abc' />
              </div>
